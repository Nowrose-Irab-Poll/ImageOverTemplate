...
let templateImage = document.getElementById('templateImage');

// Get the canvas for the edited image
let canvas = document.getElementById('canvas');
let templateCanvas = document.getElementById('templateCanvas');
...
function uploadTemplateImage(event) {
	...
	// Set the source of the image from the uploaded file
	templateImage.src = URL.createObjectURL(event.target.files[0]);

	templateImage.onload = function () {
		// Set the canvas the same width and height of the image
		templateCanvas.width = this.width;
		templateCanvas.height = this.height;
		templateCanvas.crossOrigin = "anonymous";
		applyFilter(templateContext, templateImage);
	};
...
function applyOver(x, y){
    

    const filteredImage = document.querySelector("#filtered");
    filteredImage.src = canvas.toDataURL();

    const filteredImage2 = document.querySelector("#filtered");
    templateContext.drawImage(templateImage, 0, 0);
    // templateContext.drawImage(image, x, y);
    templateContext.drawImage(filteredImage2, x, y);
    // templateContext.drawImage(target, x, y);
}

// This function is used to update the image
// along with all the filter values
function applyFilter(cont, img) {
